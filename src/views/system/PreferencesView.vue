        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white p-4">
          <div class="d-flex align-items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p class="small text-muted mb-0">Edit nama dan email akun Anda.</p>
            </div>
          </div>
          <div v-if="profileAlert" :class="['alert', profileAlertType === 'success' ? 'alert-success' : 'alert-danger', 'py-2 small mb-3']">
            {{ profileAlert }}
          </div>
          <form @submit.prevent="saveProfile" class="row g-3">
            <div class="col-md-4">
              <label class="form-label fw-semibold small">Nama</label>
              <input type="text" class="form-control rounded-3" v-model="profileForm.name" required />
            </div>
            <div class="col-md-4">
              <label class="form-label fw-semibold small">Email</label>
              <input type="email" class="form-control rounded-3" v-model="profileForm.email" required />
            </div>
            <div class="col-md-4">
              <label class="form-label fw-semibold small">Role</label>
              <input type="text" class="form-control rounded-3 bg-light" :value="currentUser?.role || '-'" disabled />
            </div>
            <div class="col-12 text-end">
              <button type="submit" class="btn btn-primary px-4 rounded-3 fw-semibold" :disabled="profileSaving">
...
export default {
  ...
  setup() {
    ...
    const profileAlertType = ref('success');
    ...
    const saveProfile = async () => {
      ...
      try {
        ...
        profileAlert.value = 'Profil berhasil diperbarui!';
        profileAlertType.value = 'success';
      } catch (e) {
        profileAlert.value = e.response?.data?.message || 'Gagal memperbarui profil.';
        profileAlertType.value = 'error';
      } finally {
        profileSaving.value = false;
        setTimeout(() => { profileAlert.value = ''; }, 3000);
      }
    };
